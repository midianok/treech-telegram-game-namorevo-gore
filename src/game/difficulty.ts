...
export function getPipeSpeed(score: number): number {
  return PIPE_SPEED - getDifficultyLevel(score) * PIPE_SPEED_STEP;
}

export function getPipeGap(score: number): number {
  return Math.max(PIPE_GAP - getDifficultyLevel(score) * PIPE_GAP_STEP, MIN_PIPE_GAP);
}

export function getPipeSpawnDelay(score: number): number {
  return Math.max(
    BASE_PIPE_SPAWN_DELAY_MS - getDifficultyLevel(score) * PIPE_SPAWN_DELAY_STEP_MS,
